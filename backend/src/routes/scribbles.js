...
scribblesRouter.get('/', requireAuth, async (req, res) => {
  const userId = Number(req.auth?.sub);

  try {
    const scribbles = await listScribbles({ userId, limit: MAX_SCRIBBLES_PER_USER });
    res.json({ scribbles, maxScribbles: MAX_SCRIBBLES_PER_USER });
  } catch (e) {
    console.error('[scribbles] list failed', { userId }, e);
    res.status(500).json({ error: 'scribbles_error', message: 'Failed to list scribbles' });
  }
});

// POST /scribbles - Create a new scribble
scribblesRouter.post('/', requireAuth, async (req, res) => {
  const userId = Number(req.auth?.sub);
  ...
  try {
    const scribble = await createScribble({ userId, imageData, authorName, expiresAt });
    res.status(201).json({ scribble });
  } catch (e) {
    console.error('[scribbles] create failed', { userId }, e);
    res.status(500).json({ error: 'scribbles_error', message: 'Failed to create scribble' });
  }
});

// DELETE /scribbles/:id - Delete a scribble
scribblesRouter.delete('/:id', requireAuth, async (req, res) => {
  const userId = Number(req.auth?.sub);
  ...
  try {
    const deleted = await deleteScribble({ userId, scribbleId });
    if (!deleted) {
      return res.status(404).json({ error: 'not_found' });
    }
    res.json({ ok: true });
  } catch (e) {
    console.error('[scribbles] delete failed', { userId, scribbleId }, e);
...
scribblesRouter.patch('/:id/pin', requireAuth, async (req, res) => {
  const userId = Number(req.auth?.sub);
  const scribbleId = Number(req.params.id);
  ...
  try {
    const updated = await pinScribble({ userId, scribbleId, pinned: parsed.data.pinned });
    if (!updated) {
      return res.status(404).json({ error: 'not_found' });
    }
    res.json({ ok: true });
  } catch (e) {
    console.error('[scribbles] pin failed', { userId, scribbleId }, e);
