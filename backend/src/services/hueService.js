...
async function getHueConfig() {
  ...
  const dbBridgeUrl = normalizeBridgeUrl(await getSetting('hue.bridgeUrl'));
  const dbAppKey = String((await getSetting('hue.appKey')) || '').trim();
  const dbAllowSelfSignedRaw = await getSetting('hue.allowSelfSigned');

  const allowSelfSigned =
    dbAllowSelfSignedRaw != null
      ? parseBoolean(dbAllowSelfSignedRaw, true)
      : parseBoolean(process.env.HUE_ALLOW_SELF_SIGNED, parseBoolean(process.env.HUE_TLS_INSECURE, true));

  const bridgeUrl = dbBridgeUrl || envBridgeUrl;
  const appKey = dbAppKey || envAppKey;
  ...
  return {
    ...
    source: dbBridgeUrl || dbAppKey ? 'db' : 'env',
  };
}
...
async function requestHue(pathname, { method = 'GET', body = null } = {}) {
  ...
  if (!cfg.configured) {
    throw new Error('hue_not_configured');
  }
  ...
  return requestJsonByUrl(url, {
    ...
    headers: {
      'hue-application-key': cfg.appKey,
    },
  });
}
...
async function pairHueBridge({ bridgeUrl } = {}) {
  ...
  if (!discovered) {
    throw new Error('hue_bridge_not_found');
  }

  const payload = await requestJsonByUrl(`${discovered}/api`, {
    method: 'POST',
    body: { devicetype: 'dashbo#server', generateclientkey: true },
    timeoutMs: cfg.timeoutMs,
    rejectUnauthorized: !cfg.allowSelfSigned,
  });
  ...
  await setSetting('hue.bridgeUrl', discovered);
  await setSetting('hue.appKey', username);
  await setSetting('hue.allowSelfSigned', cfg.allowSelfSigned ? 'true' : 'false');
...
function mapRoom(room, groupedLightsMap) {
  ...
  // Find the grouped_light service that belongs to this room
...
async function setHueRoomState({ id, on, brightness }) {
  ...
  await requestHue(`/clip/v2/resource/grouped_light/${encodeURIComponent(roomId)}`, {
    method: 'PUT',
    body,
  });
  return { ok: true };
...
async function setHueLightState({ id, on, brightness, hexColor }) {
  ...
  await requestHue(`/clip/v2/resource/light/${encodeURIComponent(lightId)}`, {
    method: 'PUT',
    body,
  });
  return { ok: true };
